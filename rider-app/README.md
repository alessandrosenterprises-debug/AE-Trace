...
The app requests a location update every five seconds and uploads it immediately when connectivity is available; Android and iOS may delay or batch location callbacks, so five seconds is a requested cadence, not a guaranteed delivery schedule. On Android, registered background location tasks are restored at boot by Expo Task Manager. This build requests `RECEIVE_BOOT_COMPLETED`; the rider must first enroll the phone, grant **Allow all the time** location permission, and keep AE-Trace unrestricted from battery optimization. Some manufacturers, including Infinix/XOS, require enabling Auto-start/background activity in their device settings. After reboot, Android starts normal boot receivers after the user unlocks the phone. A force-stop, revoked permission, disabled Location, exhausted battery, or manufacturer restriction prevents tracking until restored. Expo Go supports foreground preview only; use the native APK for background and boot-resume tracking. To control installation and app restrictions, enroll company phones with Android Enterprise as fully managed/Device Owner. AE-Trace keeps Android's visible location notification and does not bypass operating-system security.
